...
from principal.models import Provincia,Categoria,Articulo,PerfilUsuario,Comentario,Mensaje
...
from principal.forms import SignUpForm,ArticuloForm,ComentarioForm,nuevomensaje,modificarForm
...
def articulos(request):
	...
	paginator=Paginator(articulos, 4) # Muestra de 4 en 4 contactos por pagina
	...
	return render_to_response('articulos.html',{'articulos':articulos}, context_instance=RequestContext(request))
...
def id_articulo(request, id_articulo):
	...
	return render_to_response('articulo.html',{'articulo':id_articulo, 'comentarios':comentarios}, context_instance=RequestContext(request))
...
def nuevo_articulo (request):
	...
	return render_to_response('nuevoarticulo.html',{'formulario':formulario, 'usuario':usuario},context_instance=RequestContext(request))
...
def modificar_articulo(request, id_articulo):
	...
	if articulo.usuario.username == request.user.username:
		...
		return render_to_response('sinpermiso.html',{'articulo':articulo},context_instance=RequestContext(request))
...
def id_usuario(request,id_usuario):
	...
	return render_to_response('usuario.html',{'usuario':id_usuario,'usuart':usuart}, context_instance=RequestContext(request))

def busqueda(request):
	provincia=Provincia.objects.all()
	categoria=Categoria.objects.all()
	return render_to_response('busqueda.html',{'provincia':provincia, 'categoria':categoria}, context_instance=RequestContext(request))

def btit(request):
	titulo=request.GET['titulo']
	articulo=Articulo.objects.filter(titulo__contains=titulo)
	return render_to_response('btitulo.html',{'articulo':articulo}, context_instance=RequestContext(request))

def bprov(request):
	provincia=request.GET['provincia']
	articulo=Articulo.objects.filter(Provincia__nombre__contains=provincia)
	return render_to_response('bprov.html',{'articulo':articulo},context_instance=RequestContext(request))

def buscat(request):
	categoria=request.GET['categoria']
	articulo=Articulo.objects.filter(Categoria__nombre__contains=categoria)
	return render_to_response('bcat.html',{'articulo':articulo}, context_instance=RequestContext(request))
